...
public class GameServerOpenBehaviour : BehaviourBase<GameServerOpenRequest, GameServerOpenResponse>
{
  ...
  public override async Task<GameServerOpenResponse> ExecuteBehaviourAsync(
    TcpClient client, GameServerOpenRequest request)
  {
    // TODO: burda bug olabilir gameServer databaseden çektiğim için kayıt etmeyebilir
    try
    {
      var gameServer = await _gameServerRepository.GetByGuidAsync(request.ServerId);
